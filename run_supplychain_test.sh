...
# This script tests the Text2SQL system with enhanced business glossary on the supply chain schema
...
python3 tools/utils/clear_and_reload.py --tenant-id $TENANT_ID
# Alternative: direct Neo4j query to clear everything
...
python3 simulate_schema.py --tenant-id $TENANT_ID --schema-type supply_chain --schema-file ./schemas/supply_chain_simple.sql

echo ""
echo "====== STEP 3: DETECT RELATIONSHIPS ======"
python3 tools/schema/detect_relationships.py --tenant-id $TENANT_ID --use-llm

echo ""
echo "====== STEP 4: RUN DIRECT ENHANCEMENT WITH ENHANCED BUSINESS GLOSSARY ======"
python3 tools/enhance/run_direct_enhancement.py --tenant-id $TENANT_ID --use-enhanced-glossary

echo ""
echo "====== STEP 5: CHECK DATABASE & GRAPH STRUCTURE ======"
python3 tools/check/check_database.py --tenant-id $TENANT_ID
python3 tools/check/check_graph.py --tenant-id $TENANT_ID
