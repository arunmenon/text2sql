...
def parse_terms(content):
    ...
    for match in term_matches:
        ...
        if mapping_section:
            ...
            table_name = None  # Initialize to avoid UnboundLocalError
            if table_match:
                table_name = table_match.group(1).strip()
                table_mappings.append(table_name)
            
            # Extract columns from markdown format
            ...
            # Also try to extract from the custom format used in the generated glossary
            if not table_name:
                # Format: `table_name` table, columns: `column1`, `column2`
                table_columns_match = re.search(r'`(.+?)`\s+table,\s+columns:', mapping_section.group(1))
                if table_columns_match:
                    table_name = table_columns_match.group(1).strip()
                    table_mappings.append(table_name)
                    
                    # Extract columns from this format
                    cols_text = mapping_section.group(1).split('columns:')[1] if 'columns:' in mapping_section.group(1) else ""
                    col_matches = re.findall(r'`(.+?)`', cols_text)
